...
debug_logger = logging.getLogger('wlop_debug')
...
WLOP_FEATURE_AUDIT = [
    ...
    {'name': 'Transaction Management', 'features': [
        ...
        {'id': 'cancel', 'name': 'Cancel Payment', 'endpoint': 'POST /v2/{mid}/payments/{id}/cancel', 'implemented': False,
         ...
         'activation_note': 'Available with all Worldline Direct accounts. No special activation needed - implement the API call.'},
        {'id': 'refund', 'name': 'Refund Payment', 'endpoint': 'POST /v2/{mid}/payments/{id}/refund', 'implemented': False,
         ...
         'activation_note': 'Available with all accounts. Refund capabilities may need to be enabled in Merchant Portal > Configuration.'},
    ]},
    {'name': 'Tokenization', 'features': [
        {'id': 'htp', 'name': 'Hosted Tokenization Page', 'endpoint': 'POST /v2/{mid}/hostedtokenizations', 'implemented': False,
         ...
         'activation_note': 'Must be enabled in Merchant Portal. Requires PCI SAQ-A compliance for the merchant page hosting the iFrame.'},
        {'id': 'token_create', 'name': 'Create Token', 'endpoint': 'POST /v2/{mid}/tokens', 'implemented': False,
         ...
         'activation_note': 'Tokenization must be enabled on your account. Contact Worldline support or enable via Merchant Portal.'},
        {'id': 'token_delete', 'name': 'Delete Token', 'endpoint': 'DELETE /v2/{mid}/tokens/{id}', 'implemented': False,
         ...
         'activation_note': 'Part of tokenization feature. Enable tokenization first.'},
    ]},
    {'name': 'Advanced Features', 'features': [
        {'id': 'paybylink', 'name': 'Pay-by-Link', 'endpoint': 'Merchant Portal', 'implemented': False,
         ...
         'activation_note': 'Available directly in the Worldline Merchant Portal. No API integration needed - generate links from the portal UI.'},
        {'id': 'recurring', 'name': 'Recurring Payments', 'endpoint': 'POST /v2/{mid}/payments (recurring)', 'implemented': False,
         ...
         'activation_note': 'Requires tokenization to be active. Store card tokens first, then use them for subsequent payments with isRecurring flag.'},
        {'id': 'dcc', 'name': 'Dynamic Currency Conversion', 'endpoint': 'DCC via payment flow', 'implemented': False,
         ...
         'activation_note': 'Must be activated by Worldline on your account. Requires DCC agreement with your acquirer. Contact your account manager.'},
    ]},
    {'name': 'Risk & Security', 'features': [
        ...
        {'id': 'fraud', 'name': 'Fraud Detection', 'endpoint': None, 'implemented': False,
         ...
         'activation_note': 'Advanced fraud services must be activated on your account. Contact Worldline sales for fraud screening module activation.'},
    ]},
    {'name': 'OmniChannel', 'features': [
        {'id': 'omni', 'name': 'OmniChannel Integration', 'endpoint': 'Worldline POS + Direct API', 'implemented': False,
         'description': 'Bridge POS and e-commerce channels. Transfer transactions between physical retail and online for unified management.',
         'activation_note': 'Requires a Worldline Omni-Channel contract and compatible POS terminal. Contact your Worldline account manager to set up the omni-channel bridge between POS and Direct API.'},
    ]},
    {'name': 'Mail / Phone Order (MOTO)', 'features': [
        {'id': 'moto', 'name': 'MOTO Transactions', 'endpoint': 'POST /v2/{mid}/payments (MOTO)', 'implemented': False,
         'description': 'Process card-not-present transactions where card details are provided by phone or mail.',
         'activation_note': 'Requires a MOTO-enabled merchant account. Must be configured in Merchant Portal. Merchant needs PCI SAQ-C or higher. No 3DS available for MOTO transactions.'},
    ]},
]
...
def create_hosted_checkout():
    ...
    try:
        ...
        log_error('create_hosted_checkout', e, extra={'order_id': order_id})
...
def get_hc_status():
    ...
    try:
        ...
        log_error('get_hc_status', e, extra={'hc_id': hc_id})
        log_entry['status_code'] = 0
        log_entry['response'] = {'error': str(e)}
        store['logs'].append(log_entry)
        return jsonify({'success': False, 'error': str(e), 'log': log_entry}), 500
...
def create_payment_s2s():
    ...
    try:
        ...
        log_error('create_payment_s2s', e, extra={'order_id': order_id})
        log_entry['status_code'] = 0
        log_entry['response'] = {'error': str(e)}
        store['logs'].append(log_entry)
        return jsonify({'success': False, 'error': str(e), 'log': log_entry}), 500
...
def get_payment_status():
    ...
    try:
        ...
        log_error('get_payment_status', e, extra={'payment_id': payment_id})
        log_entry['status_code'] = 0
        log_entry['response'] = {'error': str(e)}
        store['logs'].append(log_entry)
        return jsonify({'success': False, 'error': str(e), 'log': log_entry}), 500
...
def capture_payment():
    ...
    try:
        ...
        log_error('capture_payment', e, extra={'payment_id': payment_id})
        log_entry['status_code'] = 0
        log_entry['response'] = {'error': str(e)}
        store['logs'].append(log_entry)
        return jsonify({'success': False, 'error': str(e), 'log': log_entry}), 500
